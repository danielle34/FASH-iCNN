"""
Training loop for Stage 2 family CSS models.

AdamW: backbones lr=5e-5 (lower than normal because pretrained), head lr=1e-3,
weight_decay=1e-3. CrossEntropyLoss(label_smoothing=0.1), mixed precision,
patience=10, max 50 epochs, checkpoint resume.
...
def make_family_optimizer(model, backbone_lr=5e-5, head_lr=1e-3, weight_decay=1e-3):
    backbone_ids = set()
    backbone_params = []
...
def train_family_model(model, train_loader, val_loader, optimizer, device,
                       max_epochs=50, patience=10, checkpoint_path=None,
                       no_resume=False):

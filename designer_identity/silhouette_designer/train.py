"""
Training loop for dual-stream face + silhouette experiments.
...
def make_optimizer(model, backbone_lr=1e-4, head_lr=1e-3, weight_decay=1e-3):
    ...
    if model.silhouette_backbone is not None:
        for p in model.silhouette_backbone.parameters():
            backbone_params.append(p)
            backbone_ids.add(id(p))
...
def train_model(model, train_loader, val_loader, optimizer, device,
    ...
    for epoch in range(start_epoch, max_epochs):
        ...
        for batch in train_loader:
            face_imgs, sil_imgs, labels = [
                b.to(device, non_blocking=True) for b in batch
            ]
            optimizer.zero_grad()
            with torch.amp.autocast("cuda", enabled=(device.type == "cuda")):
                logits = model(face_imgs, sil_imgs)
